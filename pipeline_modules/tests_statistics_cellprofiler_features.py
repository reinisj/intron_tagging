...
# DMSO wells
platemap_DMSO = platemap[platemap.Type == "DMSO"]
...
for clone_idx in tqdm(clones_metadata.index[:]):
    ...
    # use normal model predictions for DMSO wells
    predictions_DMSO = predictions_all.query(f'clone_predicted == "{clone}"')
    predictions_DMSO_clone = predictions_DMSO[predictions_DMSO.well.isin(platemap_DMSO.well)]
    ...
    cellprofiler_DMSO_clone = cellprofiler.merge(predictions_DMSO_clone[["image", "ObjectNumber_cell"]])
    cp_controls = cellprofiler_DMSO_clone[features_to_test]
    ...
    median_matrix = mean_matrix.copy()
    iqr_matrix = mean_matrix.copy()
    mad_matrix = mean_matrix.copy()
    ...
    for well in platemap.well[:]:
        ...
        medians = np.array(cp_well.median()).reshape((1,len(features_to_test)))
        # throws an exception if there are no cells -> return nans like the 4 lines above do
        try:
            iqrs = scipy.stats.iqr(cp_well, axis=0).reshape((1,len(features_to_test)))
            mads = scipy.stats.median_abs_deviation(cp_well).reshape((1,len(features_to_test)))
        except AttributeError:
            iqrs = np.array([np.nan for _ in features_to_test]).reshape((1,len(features_to_test)))
            mads = np.array([np.nan for _ in features_to_test]).reshape((1,len(features_to_test)))
            
        pvals_matrix.loc[well] = pvals
        mean_matrix.loc[well] = means
        stddev_matrix.loc[well] = stds
        median_matrix.loc[well] = medians
        iqr_matrix.loc[well] = iqrs
        mad_matrix.loc[well] = mads
    ...
    stats_controls = stats_controls.merge(cp_controls.median().rename("median_control").reset_index(), on="index")
    stats_controls["iqr_control"] = scipy.stats.iqr(cp_controls, axis=0)
    stats_controls["mad_control"] = scipy.stats.median_abs_deviation(cp_controls)
    ...
    median_matrix_melted = pd.melt(median_matrix.reset_index(), id_vars="well", value_vars=median_matrix.columns, value_name = "median_treated")
    std_matrix_melted = pd.melt(stddev_matrix.reset_index(), id_vars="well", value_vars=stddev_matrix.columns, value_name = "std_treated")
    iqr_matrix_melted = pd.melt(iqr_matrix.reset_index(), id_vars="well", value_vars=iqr_matrix.columns, value_name = "iqr_treated")
    mad_matrix_melted = pd.melt(mad_matrix.reset_index(), id_vars="well", value_vars=iqr_matrix.columns, value_name = "mad_treated")

    # merge in to one a single table
    result_clone = mean_matrix_melted.merge(std_matrix_melted, on=["well", "variable"]).merge(pvals_matrix_melted, on=["well", "variable"])
    result_clone = result_clone.merge(median_matrix_melted, on=["well", "variable"]).merge(iqr_matrix_melted, on=["well", "variable"]).merge(mad_matrix_melted, on=["well", "variable"])
...
columns_reorder = ["protein", "sgRNA", "clone", "frame", "CompoundName", "AssayConc_uM", "well", "variable", "pval", "fold_change", 
                   "mean_treated", "std_treated", "median_treated", "iqr_treated", "mad_treated", "n_cells_treated", 
                   "mean_control", "std_control", "median_control", "iqr_control", "mad_control", "n_cells_control", ]
